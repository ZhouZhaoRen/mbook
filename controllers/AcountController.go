...
func (c *AccountController) DoRegist() {
	...
	if password1 != password2 {
		c.JsonResult(1, "登录密码与确认密码不一致") // 不用进行页面跳转，所以返回json数据
	}
	...
	member.Avatar = common.DefaultAvatar() // 默认头像
	member.CreateAt = 0
	member.Email = email
	member.Status = 0
...
func (c *AccountController) Logout() {
	c.SetMember(models.Member{}) // memberID为0，此时就是注销session和cookie
	c.SetSecureCookie(common.AppKey(), "login", "", -3600)
	c.Redirect(beego.URLFor("AccountController.Login"), 302) // 重定向到登录首页
}
